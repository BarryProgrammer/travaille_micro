...
public class ProjectExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, HttpHeaders headers, HttpStatusCode status, WebRequest request){
        ...
        ex.getBindingResult().getFieldErrors().forEach(error->fieldError.add(error.getDefaultMessage()));
        ApiError body = new ApiError(status.value(), HttpStatus.valueOf(status.value()).name(), "Validation failed for one or more field", fieldError);
    ...
    protected ResponseEntity<Object> handleExceptionInternal(Exception ex, Object body, HttpHeaders headers, HttpStatusCode statusCode, WebRequest request){
        log.error("An Error occured while proceccing the request.", ex);
